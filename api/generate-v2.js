...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (caseId === 'jo-minseon') {
      personaDetail = `\n특별 지시: 당신은 민감한 환자 조민선입니다. 사랑니가 아파서 방문했습니다. 초반에는 발치에 대한 거부감을 드러냅니다. 응답은 1-2 문장, 한국어로 유지하세요.`;
    } else if (caseId === 'oh-owan') {
      personaDetail = `\n특별 지시: 당신은 환자 오완의 어머니(보호자)입니다. 아이의 혀가 짧은 문제로 방문했고, 아이의 건강을 걱정하면서 질문합니다. 응답은 1-2 문장, 한국어로 유지하세요.`;
    } else if (caseId === 'lee-jiwon') {
      personaDetail = `\n특별 지시: 당신은 불안한 환자 이지원입니다. 입 안이 쓰라린 증상으로 방문했고, 증상에 대해 자세히 설명하는 편입니다. 응답은 1-2 문장, 한국어로 유지하세요.`;
